...
def _rust_proto_aspect_common(target, ctx, is_upb):
    ...
    extra_rustc_dep_infos = [
        ...
        for dep in ctx.attr._extra_rustc_deps
    ]
    ...
    dep_variant_info = _compile_rust(
        ...
        deps = [dep_variant_info_for_runtime] + dep_variant_info_for_native_gencode + dep_variant_infos + extra_rustc_dep_infos,
        runtime = runtime,
    )
    return [RustProtoInfo(
...
def _make_proto_library_aspect(is_upb):
    return aspect(
        ...
        attrs = {
            ...
            "_extra_rustc_deps": attr.label_list(
                default = [
                    Label("//third_party/crubit/support:forward_declare"),
                ],
            ),
